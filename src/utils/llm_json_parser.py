...
class LLMJsonParser:
    ...
    def invoke_llm_with_retry(self, prompt, validation_type="summary", max_retries=2):
        ...
        while attempt <= max_retries:
            try:
                logger.info(
                    f"LLM attempt {attempt + 1}/{max_retries + 1} (validation_type: {validation_type})"
                )
                ...
                parsed_data = self._parse_and_validate_json(
                    response_text, validation_type
                )
        ...
        raise ValueError(
            f"Failed to generate {validation_type} after {max_retries + 1} attempts"
        )
    ...
    def _validate_summary_json(self, parsed):
        ...
        for heading_key, heading_text in parsed["summary"].items():
            if not isinstance(heading_text, str) or len(heading_text.strip()) < 10:
                raise ValueError(
                    f"'{heading_key}' must be a non-empty string with meaningful content"
                )
        ...
        for i, question in enumerate(parsed["questions"]):
            ...
            for field in required_fields:
                if field not in question:
                    raise ValueError(
                        f"questions[{i}] missing required field: '{field}'"
                    )

            # Validate id
            expected_id = str(i + 1)
            if not isinstance(question["id"], str) or question["id"] != expected_id:
                raise ValueError(
                    f"questions[{i}] 'id' must be string matching position (expected '{expected_id}')"
                )

            # Validate question_text
            if (
                not isinstance(question["question_text"], str)
                or len(question["question_text"].strip()) < 5
            ):
                raise ValueError(
                    f"questions[{i}] 'question_text' must be a non-empty string"
                )
            ...
            if option_keys != expected_keys:
                raise ValueError(
                    f"questions[{i}] 'options' must have exactly keys: A, B, C, D"
                )

            for option_key, option_text in question["options"].items():
                if not isinstance(option_text, str) or len(option_text.strip()) < 2:
                    raise ValueError(
                        f"questions[{i}] option '{option_key}' must be a non-empty string"
                    )

            # Validate correct_answer
            if question["correct_answer"] not in ["A", "B", "C", "D"]:
                raise ValueError(
                    f"questions[{i}] 'correct_answer' must be one of: A, B, C, D"
                )

            # Validate explanation
            if (
                not isinstance(question["explanation"], str)
                or len(question["explanation"].strip()) < 5
            ):
                raise ValueError(
                    f"questions[{i}] 'explanation' must be a non-empty string"
                )
    ...
    def _validate_ppt_json(self, parsed):
        ...
        for i, slide in enumerate(parsed["slides"]):
            ...
            if not isinstance(slide["slide_number"], int) or slide["slide_number"] != (
                i + 1
            ):
                raise ValueError(
                    f"slides[{i}] 'slide_number' must be an integer matching position (expected {i + 1})"
                )
            ...
            if slide["slide_type"] not in valid_types:
                raise ValueError(
                    f"slides[{i}] 'slide_type' must be one of: {', '.join(valid_types)}"
                )
            ...
            if slide["slide_type"] == "title":
                if (
                    not isinstance(slide["subtitle"], str)
                    or len(slide["subtitle"].strip()) < 2
                ):
                    raise ValueError(
                        f"slides[{i}] 'subtitle' must be a non-empty string"
                    )
            else:
                # Validate bullet_points for non-title slides
                if not isinstance(slide["bullet_points"], list):
                    raise ValueError(
                        f"slides[{i}] 'bullet_points' must be an array/list"
                    )

                if len(slide["bullet_points"]) < 1:
                    raise ValueError(
                        f"slides[{i}] 'bullet_points' must have at least 1 item"
                    )

                for j, point in enumerate(slide["bullet_points"]):
                    if not isinstance(point, str) or len(point.strip()) < 2:
                        raise ValueError(
                            f"slides[{i}] bullet_points[{j}] must be a non-empty string"
                        )
    ...
    def _validate_exercise_extraction_json(self, parsed):
        ...
        for i, section in enumerate(parsed["exercises"]):
            ...
            if "section_title" not in section:
                raise ValueError(
                    f"exercises[{i}] missing required field: 'section_title'"
                )
            ...
            if (
                not isinstance(section["section_title"], str)
                or len(section["section_title"].strip()) < 1
            ):
                raise ValueError(
                    f"exercises[{i}] 'section_title' must be a non-empty string"
                )
            ...
            if len(section["questions"]) < 1:
                raise ValueError(
                    f"exercises[{i}] 'questions' must have at least 1 question"
                )
            ...
            for j, question in enumerate(section["questions"]):
                if not isinstance(question, dict):
                    raise ValueError(
                        f"exercises[{i}] questions[{j}] must be an object/dict"
                    )
                ...
                for field in required_fields:
                    if field not in question:
                        raise ValueError(
                            f"exercises[{i}] questions[{j}] missing required field: '{field}'"
                        )

                # Validate question_number
                if (
                    not isinstance(question["question_number"], str)
                    or len(question["question_number"].strip()) < 1
                ):
                    raise ValueError(
                        f"exercises[{i}] questions[{j}] 'question_number' must be a non-empty string"
                    )
                ...
                if (
                    ...
                    raise ValueError(
                        f"exercises[{i}] questions[{j}] 'question_text' must be a meaningful string (at least 5 characters)"
                    )

                # Validate type
                if question["type"] not in valid_types:
                    raise ValueError(
                        f"exercises[{i}] questions[{j}] 'type' must be one of: {', '.join(valid_types)}"
                    )

                # Validate based_on_image
                if not isinstance(question["based_on_image"], bool):
                    raise ValueError(
                        f"exercises[{i}] questions[{j}] 'based_on_image' must be a boolean (true/false)"
                    )

                # Validate answer field exists (can be null initially)
                if "answer" not in question:
                    raise ValueError(
                        f"exercises[{i}] questions[{j}] 'answer' field is required (can be null)"
                    )
    ...
    def _validate_exercise_answering_json(self, parsed):
        ...
        for i, section in enumerate(parsed["exercises"]):
            ...
            for j, question in enumerate(section["questions"]):
                ...
                if (
                    not isinstance(question["answer"], str)
                    or len(question["answer"].strip()) < 1
                ):
                    raise ValueError(
                        f"exercises[{i}] questions[{j}] 'answer' must be a non-empty string"
                    )
                ...
                if (
                    ...
                    and question["answer"]
                    != "Cannot answer - question requires image reference"
                ):
                    logger.warning(
                        f"exercises[{i}] questions[{j}] is marked as image-based but answer is not the standard message"
                    )
