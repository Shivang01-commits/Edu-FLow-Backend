...
class SummaryService:
    ...
    def summarize_chapter(self, metadata):
        ...
        try:
            ...
            summary_data = self._invoke_llm_with_retry(prompt, max_retries=self.max_retries)
    ...
    def _repair_json(self, response_text):
        ...
        response_text = ''.join(char if ord(char) >= 32 or char in '\n\r\t' else '' 
        ...
        if response_text.count('"') % 2 != 0:
            logger.warning("Odd number of quotes detected - JSON might have unclosed string")
    ...
    def _validate_json_structure(self, parsed):
        ...
        for i, point in enumerate(parsed["key_points"]):
            ...
            if not isinstance(point["heading"], str) or len(point["heading"]) < 3:
                raise ValueError(f"key_points[{i}]['heading'] must be a non-empty string")
            
            if not isinstance(point["description"], str) or len(point["description"]) < 10:
                raise ValueError(f"key_points[{i}]['description'] must be a non-empty string")
