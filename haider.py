"""
EduLens Backend v4 — robust background task + /status polling
"""
...
async def lifespan(app: FastAPI):
    logger.info(f"EduLens v4 starting. LLM: {config.LLM_BASE_URL} model={config.LLM_MODEL}")
    yield
...
def extract_pdf_text(pdf_bytes: bytes) -> tuple[str, int]:
    try:
        ...
        if page_count > config.MAX_PDF_PAGES:
            raise ValueError(f"PDF has {page_count} pages. Max is {config.MAX_PDF_PAGES}.")
...
def split_into_chunks(text: str, chunk_size: int) -> list[str]:
    ...
    while remaining:
        if len(remaining) <= chunk_size:
            chunks.append(remaining.strip()); break
...
def parse_quiz(raw: str) -> list[QuizQuestion]:
    ...
    for block in re.split(r"\n(?=\d+[\.\)])", raw):
        ...
        for line in lines[1:]:
            ...
            if m:  options.append(QuizOption(label=m.group(1).upper(), text=m.group(2).strip()))
            if am: answer_label = am.group(1).upper()
            if em: explanation  = em.group(1).strip()
        if q_line and len(options) >= 2 and answer_label:
            if not any(q.question.lower() == q_line.lower() for q in questions):
                questions.append(QuizQuestion(
                    question=q_line, options=options, answer=answer_label,
...
def gen_summary(chunks: list[str]) -> tuple[str, list[str]]:
    ...
    for i, chunk in enumerate(use):
        ...
        raw = call_llm_sync(f"""Part {i+1} of {len(use)} of an academic chapter.
...
Respond now:""", max_tokens=700)
        s_m = re.search(r"SUMMARY:\s*(.*?)(?=KEY CONCEPTS:|$)", raw, re.DOTALL | re.IGNORECASE)
        c_m = re.search(r"KEY CONCEPTS:\s*(.*?)$", raw, re.DOTALL | re.IGNORECASE)
        all_s.append(s_m.group(1).strip() if s_m else raw[:500])
        if c_m: all_c.extend(parse_concepts(c_m.group(1)))
...
def gen_qa(chunks: list[str]) -> list[ChapterQA]:
    ...
    for i, chunk in enumerate(chunks):
        ...
        raw = call_llm_sync(f"""Read this academic chapter section.
...
Q3: [question]
A3: [2-4 sentence answer]""", max_tokens=800)
        new = parse_qa(raw)
        added = 0
        for p in new:
            if not any(e.question.lower() == p.question.lower() for e in all_pairs):
                all_pairs.append(p); added += 1
...
def gen_quiz(chunks: list[str]) -> list[QuizQuestion]:
    ...
    for i, chunk in enumerate(use):
        ...
        raw = call_llm_sync(f"""Create exactly {count} multiple-choice questions from this chapter section.
...
def run_analysis_thread(pdf_bytes: bytes, filename: str):
    ...
    try:
        ...
        push(f"📊 Extracted {word_count:,} words from {page_count} pages → {len(chunks)} sections to process")
        ...
        result = AnalysisResult(
            filename=filename, page_count=page_count, word_count=word_count,
            summary=summary, key_concepts=key_concepts, chapter_qa=chapter_qa,
            quiz=quiz, processing_time_seconds=elapsed,
        )
        job_store.update({"result": result.model_dump(), "status": "done", "finished_at": time.time()})
        push(f"🎉 All done in {round(elapsed/60, 1)} minutes!")

    except Exception as e:
        job_store.update({"status": "error", "error": str(e), "finished_at": time.time()})
        push(f"❌ Error: {e}")
...
@app.post("/analyze")
async def analyze_chapter(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    """Upload PDF — analysis starts immediately in background. Poll /status for updates."""
    if job_store["status"] == "running":
        raise HTTPException(409, "An analysis is already running. Please wait for it to finish.")
    if not file.filename or not file.filename.lower().endswith(".pdf"):
        raise HTTPException(400, "Only PDF files are accepted.")
    pdf_bytes = await file.read()
    size_mb = len(pdf_bytes) / (1024 * 1024)
    if size_mb > config.MAX_PDF_SIZE_MB:
        raise HTTPException(400, f"File too large ({size_mb:.1f} MB). Max is {config.MAX_PDF_SIZE_MB} MB.")

    # Use BackgroundTasks + threading to avoid asyncio.create_task issues with --reload
    def start():
        t = threading.Thread(target=run_analysis_thread, args=(pdf_bytes, file.filename), daemon=True)
        t.start()

    background_tasks.add_task(start)
    return {"message": "Analysis started. Poll /status for progress.", "filename": file.filename}
...
async def health():
    ...
    try:
        ...
        llm_ok = r.status_code == 200; llm_note = "reachable"
    except Exception as e:
        llm_note = str(e)
    return {"status": "ok", "llm_reachable": llm_ok, "llm_note": llm_note, "job_status": job_store["status"]}


@app.get("/")
async def root():
    return {"message": "EduLens API v4 running. POST /analyze then poll /statuss